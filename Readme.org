...
[[https://github.com/astral-sh/uv][uv]] is assumed to be installed on every machine this config runs on, so there is no
fallback to the stdlib =venv= module.

To set up a new project, run =M-x ygg/python-new-project= and give it a
directory. It creates the directory if needed, runs =uv init --app --no-package=
(a flat layout: =main.py=, =pyproject.toml=, =.python-version=, a README, and a git
repo — no =src/= directory or build backend), then =uv add --dev= to put
=python-lsp-server=, =python-lsp-ruff=, =black=, and =isort= in the project's
dev dependency group, which also creates =.venv= and =uv.lock=. It then activates
the venv via =pyvenv= and opens =dired= on the project root, all asynchronously
so Emacs doesn't block. Add real dependencies afterwards with =uv add <pkg>=.
...
venv's binaries automatically via the hook.
...
python-lsp-server/python-lsp-ruff/black/isort as dev dependencies
(which creates .venv), activates the venv with pyvenv, and opens
dired on DIR.  Runs asynchronously."
  ...
  (let* ((default-directory dir)
         ...
         (script "uv init --app --no-package && uv add --dev python-lsp-server python-lsp-ruff black isort")
