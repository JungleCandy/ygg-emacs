...
regardless of which one made it. It installs =python-lsp-server[all]=,
=black=, and =isort= into the new venv, activates it via =pyvenv=, and opens
...
python-lsp-server[all]/black/isort into it, activates the venv
  ...
  (let* ((default-directory dir)
         ...
         (script
          (if (executable-find "uv")
              (format "uv venv %s && uv pip install --python %s/bin/python \"python-lsp-server[all]\" black isort"
                      (shell-quote-argument venv) (shell-quote-argument venv))
            (format "%s -m venv %s && %s/bin/pip install \"python-lsp-server[all]\" black isort"
