...
(use-package lsp-mode
  ...
  (lsp-before-save-edits nil)) ;; Let blacken/isort own formatting, not the LSP server
...
Uses `eglot` (built into Emacs) rather than `lsp-mode`+`lsp-ui`, matching the lighter-weight approach used for [[*Rust][Rust]] below — it only starts a server when =clangd= is actually found on =$PATH=, so this is safe to keep even on a machine without it installed:
#+begin_src emacs-lisp
...
That's it — =lsp-mode=, =blacken-mode=, and =py-isort= all pick up the
venv's binaries automatically via the hook, whether =uv= made it or not.
=lsp-before-save-edits= is set to =nil= so the LSP server doesn't fight
=blacken-mode= over formatting on save. =py-isort= runs with =--profile=black= so
