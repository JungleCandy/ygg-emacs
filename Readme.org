...
*** Rust
After installing Rust via rustup, the rust-analyzer component must be added explicitly — the binary at =~/.cargo/bin/rust-analyzer= is only a proxy shim and will fail without it:

=rustup component add rust-analyzer=

*** Python
[[https://github.com/astral-sh/uv][uv]] is assumed to be installed on every machine. Install the language server ruff and pylint as global tools, so scripts outside any project still get completion, diagnostics and format-on-save:
...
The plugin has to come in with =--with=: =pylsp= loads plugins from its own environment, so a separate =ruff= install doesn't give it ruff diagnostics. The standalone =ruff= is only for the command line.
...
#+begin_src emacs-lisp
;; Disable native JIT compilation to avoid libgccjit/linker issues on macOS
...
;; Make M-x shell start an interactive login zsh. (Eshell is pure Elisp and
;; doesn't read shell-file-name/explicit-*-args at all, so this is M-x
;; shell-only, despite appearances.)
...
First, create a convenience function to setup the cursor I like with Dracula Pro theme

#+begin_src emacs-lisp
(defun ygg/setup-dracula-cursor ()
  "Set a gold bar cursor for Dracula Pro."
...
*** Compilation
Settings for =M-x compile= (and anything built on it, like =projectile-compile-project=).
#+begin_src emacs-lisp
;; Follow the build output as it runs, but stop auto-scrolling once an
;; error shows up so it doesn't scroll past what you need to read.
(setq compilation-scroll-output 'first-error)

;; Render ANSI color codes from compilers/build tools instead of showing
;; raw escape sequences.
(use-package ansi-color
  :ensure nil
  :hook (compilation-filter . ansi-color-compilation-filter))

;; Auto-close the compilation window on success (after a brief pause to see
;; "finished"), but leave it open on failure so the error is still visible.
;; Uses quit-window rather than delete-windows-on: if *compilation* reused
;; an existing window (e.g. your other pane) instead of splitting a new
;; one, delete-windows-on would delete that window outright, taking your
;; other pane with it. quit-window instead restores whatever was in that
;; window before, via the quit-restore parameter display-buffer sets up.
(defun ygg/quit-compilation-window (buffer)
  (let ((window (get-buffer-window buffer)))
    (when window
      (quit-window nil window))))

(defun ygg/close-compilation-window-on-success (buffer string)
  (if (string-match "^finished" string)
      (run-with-timer 1 nil #'ygg/quit-compilation-window buffer)
    (message "Compilation failed.")))

(add-hook 'compilation-finish-functions #'ygg/close-compilation-window-on-success)
#+end_src

*** Dired
Most frequently used commands (all built in, no custom bindings needed):
...
| m        | Mark file                                          |
| u        | Unmark file                                        |
| U        | Unmark all in buffer                               |
| t        | Toggle all marks                                   |
| % m      | Mark files matching regexp                         |
| d        | Flag for deletion                                  |
| x        | Execute flagged deletions                          |
| D        | Delete marked files (prompts, no need to flag first) |
...
| (        | Toggle detailed listing (permissions, size, etc.)  |
| s        | Cycle sort order                                   |
| w        | Copy file name to kill ring                        |
| C-x C-q  | Edit filenames directly as text (wdired)           |

#+begin_src emacs-lisp
;; Reuse a single Dired buffer when navigating into a directory (or back up
;; with ^) instead of stacking up a new buffer per directory.
(setq dired-kill-when-opening-new-dired-buffer t)
#+end_src

** Packages
*** exec-path-from-shell
On macOS, GUI Emacs doesn't inherit the shell's PATH. This ensures tools installed
via Homebrew, rustup, etc. are visible to Emacs.
#+begin_src emacs-lisp
...
| Key / Command      | Action                                            |
|---------------------+----------------------------------------------------|
| M-x eshell          | Open (or switch to) the Eshell buffer             |
| TAB                 | Complete file/command at point                    |
| M-p / M-n           | Previous / next input in history                  |
| M-r                 | isearch backward through input history            |
| C-c C-p / C-c C-n   | Jump to previous / next prompt                    |
| C-c C-c             | Interrupt the currently running command           |
| C-c C-u             | Kill the current input line                       |
| C-c C-o             | Delete the last command's output                  |
| C-d                 | Send EOF on empty input, else delete-char          |
...
***** eat
Eshell handles ordinary commands fine, but falls down on genuinely full-screen/curses programs. Eat is a fast, pure-Elisp terminal emulator (no native module to compile, unlike =vterm=) that plugs into Eshell for exactly that case — Eshell already has a default list of commands it treats as "visual" (=vim=, =nvim=, =top=, =htop=, =tmux=, =less=, etc.); =ssh= is added to that list below since it isn't included by default. Eat is also usable standalone as a full terminal via =M-x eat=.

Default keybindings while in a visual command (or a standalone =M-x eat= buffer), in the default "semi-char" mode:

| Key               | Action                                              |
|--------------------+------------------------------------------------------|
| (most keys)        | Passed straight through to the program              |
| C-c C-e            | Switch to "emacs" mode — normal Emacs editing/selection |
| C-c M-d            | Switch to "char" mode — full passthrough, even C-x/M-x |
| C-c C-j            | Switch back to "semi-char" mode (the default)       |
| C-c C-l            | Switch to "line" mode — Emacs-style line editing    |
| C-c C-k            | Kill the process                                    |
| C-c C-p / C-c C-n  | Jump to previous / next shell prompt                |

#+begin_src emacs-lisp
(use-package eat
  :ensure t
  :config
  ;; eshell-visual-commands isn't defined until Eshell's em-term module is
  ;; loaded, so pull it in explicitly rather than relying on load order.
  (require 'em-term)
  ;; Run Eshell's "visual" commands through Eat's terminal emulation
  ;; instead of the older term-based handling.
  (eat-eshell-visual-command-mode 1)
  (add-to-list 'eshell-visual-commands "ssh")
  ;; Eat ships terminfo databases pre-compiled for the build machine's
  ;; ncurses. On macOS that database is incompatible and makes Backspace
  ;; render as a blank character instead of erasing
  ;; (https://codeberg.org/akib/emacs-eat/issues/45). Recompile it locally
  ;; with the system's own `tic`, once per Eat version.
  (let ((marker (expand-file-name ".compiled-for-this-system"
...
|---------------+----------------------+------------------------------------------------------|
| **Compile**   | =C-c C-c=            | Run default command (LatexMk)                        |
|               | =C-c C-a=            | Compile and open viewer                              |
|               | =C-c C-v=            | View PDF in PDF Tools at cursor position (SyncTeX)   |
|               | =C-c C-l=            | Show compilation log                                 |
|               | =C-c `=              | Jump to next error                                   |
|---------------+----------------------+------------------------------------------------------|
| **Insert**    | =C-c C-e=            | Insert / close environment                           |
|               | =C-c C-s=            | Insert section heading                               |
|               | =C-c C-m=            | Insert macro                                         |
|               | =C-c C-j=            | Insert list item                                     |
|---------------+----------------------+------------------------------------------------------|
| **Fonts**     | =C-c C-f C-b=        | Bold                                                 |
|               | =C-c C-f C-i=        | Italic                                               |
|               | =C-c C-f C-e=        | Emphasize                                            |
|               | =C-c C-f C-t=        | Typewriter / monospace                               |
|---------------+----------------------+------------------------------------------------------|
| **Math**      | =`= (backtick)       | Prefix for LaTeX-math-mode shortcuts                 |
|               | =`a=                 | \alpha                                                    |
|               | =`b=                 | \beta                                                    |
|               | =`g=                 | \gamma                                                    |
|               | =`d=                 | \delta                                                    |
...
|               | =`i=                 | \infty                                                    |
|---------------+----------------------+------------------------------------------------------|
| **RefTeX**    | =C-c (=              | Insert label                                         |
|               | =C-c )=              | Insert reference                                     |
|               | =C-c [=              | Insert citation                                      |
|               | =C-c ==              | Show table of contents                               |
|               | =C-c &=              | View cross-reference at point                        |
|---------------+----------------------+------------------------------------------------------|
| **PDF Tools** | =n= / =p=            | Next / previous page                                 |
|               | =+= / =-=            | Zoom in / out                                        |
|               | =C-c C-g=            | Jump to source line (inverse SyncTeX, in PDF buffer) |
...
***** Org-roam
Build a cross-platform “second brain” (macOS + Linux) backed by a Git repo.
...
|---------+--------------------------------+------------------------------------------------------|
| C-c n f | org-roam-node-find             | Find a node; creates it if it doesn’t exist.         |
...
| C-c n d | org-roam-dailies-capture-today | Open/create today’s daily note.                      |
| C-c n o | ygg/org-roam-find-orphans      | List nodes with no links in or out.                  |
...
(defun ygg/org-roam-find-orphans ()
    "List org-roam nodes with no links in or out."
    (interactive)
    (let ((orphans (org-roam-db-query
                    [:select [nodes:title nodes:file]
                     :from nodes
                     :left-join links :on (= nodes:id links:source)
                     :where (= links:source nil)])))
      (if orphans
          (message "Orphaned nodes:\n%s"
                   (mapconcat (lambda (r) (format "- %s" (car r))) orphans "\n"))
        (message "No orphaned nodes found."))))

  (global-set-key (kbd "C-c n o") #'ygg/org-roam-find-orphans)
  ...
  (setq org-roam-capture-templates
        '(("d" "default" plain "%?"
          ...
          ("s" "WWDC Session" plain
           "\n* Overview\n%?\n\n* Key Points\n\n* Questions & Follow-ups\n\n* Links\n"
           :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org"
                              "#+title: ${title}\n#+date: %U\n#+filetags: :wwdc:wwdc26:\n\n")
           :unnarrowed t)
...
****** Ox-hugo capture templates
These are customised for my specific hugo blogs.

| C-c c b p | New post for abizern.org |
| C-c c b t | New post for abizern.dev |

#+begin_src emacs-lisp
(setq org-capture-templates
      '(("b" "Blog Post")

        ("bp" "Personal blog post" entry
         (file+olp "~/Sites/abizern-org/Content.org" "Posts")
         "* %^{Title}\n:PROPERTIES:\n:EXPORT_FILE_NAME: %\\1\n:EXPORT_DATE: %<%Y-%m-%d>\n:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :meta true :math false\n:END:\n\n%?"
         :empty-lines 1)

        ("bt" "Technical blog post (abizern.dev)" entry
         (file+olp "~/Sites/Abizern.github.io/Content.org" "Posts")
         "* %^{Title}\n:PROPERTIES:\n:EXPORT_FILE_NAME: %\\1\n:EXPORT_DATE: %<%Y-%m-%d>\n:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :meta true :math false\n:END:\n\n%?"
         :empty-lines 1)))
#+end_src
...
#+begin_src emacs-lisp
  (defun ygg/apply-editorconfig-style ()
    ...
    (cond
     ...
     ;; Org — strict 8-wide tabs
     ((derived-mode-p 'org-mode)
      (setq indent-tabs-mode nil
            tab-width 8))

     ;; Markdown / text — allow spacey editing, usually 2
...
(use-package lsp-mode
  ...
  (lsp-before-save-edits nil) ;; Don't auto-format on save; Python triggers ruff formatting explicitly instead
  ;; clangd's on-type formatting fires asynchronously and, with no
  ;; .clang-format file in the project, falls back to LLVM style — it
  ;; silently overwrites cc-mode's correct 1tbs indentation a moment after
  ;; you type, which looks like the cursor "jumping" to column 0.
...
(use-package lsp-ui
  ...
  :custom
  (lsp-ui-doc-enable t)
  (lsp-ui-doc-show-with-cursor nil) ;; Only show doc popup on mouse hover, not while typing/moving point
...
*** C/C++ Development
`clangd` is on =$PATH= (=/usr/bin/clangd=), so =lsp-mode='s built-in clangd client works with no extra configuration — =c-mode= / =c++-mode= are hooked into ~lsp~ above.

Indentation style is One True Brace Style (OTBS/1TBS) — every opening brace, including function definitions, stays cuddled at the end of the line, and `else`/`while`/`catch` cuddle onto the preceding closing brace. `cc-mode` has no built-in style for this (the closest, =linux=, still puts a function's opening brace on its own line), so it's defined as a custom style based on =linux=:
#+begin_src emacs-lisp
(use-package cc-mode
  :ensure nil
  :config
  (c-add-style "1tbs"
               '("linux"
                 ...
                 (c-offsets-alist . ((substatement-open . 0)
                                      (statement-cont . +)
                                      (case-label . +)))))
  (setq c-default-style '((c-mode . "1tbs")
                           (c++-mode . "1tbs")
                           (other . "gnu"))))
#+end_src
...
*** Rust
Portable Rust configuration for Emacs 30.2 and environments where Rust
may or may not be installed.

Useful Emacs keybindings and commands for working with Rust, rust-analyzer,
Cargo, and Eglot. Safe to keep in your config even on machines without Rust.
...
;; Install the Rust tree-sitter grammar if not already present
...
;; rust-ts-mode is built into Emacs 29+, no :ensure needed
...
*** TOML
Major mode for editing TOML files, notably =Cargo.toml= and =Cargo.lock=.
...
*** Python Development
*Once per machine*, run these two commands (also in the Preamble):

=uv tool install python-lsp-server --with python-lsp-ruff=

=uv tool install ruff=

Without them, Python buffers outside a project get no language server and no format-on-save.

Everything runs through one language server: =pylsp=, hooked into =lsp-mode=
above like Swift, Go, and C/C++, with the
[[https://github.com/python-lsp/python-lsp-ruff][python-lsp-ruff]] plugin. Ruff does the linting, formatting and import
sorting; Jedi (part of =pylsp=) does completion, hover, definitions, references
and rename. That is deliberately all — not =python-lsp-server[all]=, whose extras
(pylint, flake8, rope, yapf, autopep8, ...) duplicate ruff. [[https://github.com/astral-sh/uv][uv]] is assumed
to be installed on every machine, so there is no fallback to the stdlib =venv=.

There are two ways to work:

- *Scripts and one-off files* use the global =pylsp= from =uv tool install= (see
  the Preamble), found in =~/.local/bin=. Jedi can't see third-party packages
  here, which is fine for standalone scripts.
- *Projects* get their own =pylsp= in the project's environment, so Jedi runs in
  the same process as the project's dependencies and can complete third-party
  code. =ygg/auto-pyvenv-activate= puts the project's =.venv/bin= first on
  =exec-path=, so the project's copy wins over the global one. A project created
  outside Emacs (a plain =uv init=) has no =pylsp= in its venv and quietly
  falls back to the global one, without dependency completion.

To set up a project, run =M-x ygg/python-new-project= and give it a directory.
It creates the directory if needed, runs =uv init --app --no-package= (a flat
layout: =main.py=, =pyproject.toml=, =.python-version=, a README, and a git repo
— no =src/= directory or build backend), then =uv add --dev= to put
=python-lsp-server= and =python-lsp-ruff= in the dev dependency group, which
also creates =.venv= and =uv.lock=. It then activates the venv via =pyvenv= and
opens =dired= on the project root, all asynchronously so Emacs doesn't block.
Add real dependencies afterwards with =uv add <pkg>=.

On save, =ygg/python-lsp-tidy= sorts imports (=lsp-organize-imports=) and then
formats the buffer (=lsp-format-buffer=), both through ruff. =lsp-before-save-edits=
stays =nil= (see LSP Support above) so the server never auto-formats on its own;
formatting is triggered explicitly instead. Formatting uses ruff's defaults (88
columns, black-compatible style); change them per project under =[tool.ruff]=
in =pyproject.toml=. Errors are demoted to messages, because an error in
=before-save-hook= would otherwise abort the save.

*Keybindings and commands.* Everything here is standard =python.el=, Xref, or =lsp-mode= — nothing is custom-bound except where noted.

| Category       | Command / Keybinding          | What It Does                                              |
|----------------+-------------------------------+-------------------------------------------------------------|
| **REPL**       | `C-c C-p`                     | Start a Python REPL (see the note below)                  |
|                | `C-c C-z`                     | Switch between the buffer and the REPL                    |
|                | `C-c C-c`                     | Send the whole buffer (skips `if __name__ == "__main__"`) |
|                | `C-u C-c C-c`                 | Send the whole buffer including the `__main__` block      |
|                | `C-c C-r`                     | Send the active region                                    |
|                | `C-M-x`                       | Send the function under point                             |
|                | `C-c C-e`                     | Send the statement under point                             |
|                | `C-c C-l`                     | Send a file to the REPL                                    |
|----------------+-------------------------------+-------------------------------------------------------------|
| **Navigation** | `M-.`                         | Jump to definition (lsp-mode)                              |
|                | `M-,`                         | Jump back                                                   |
|                | `M-?`                         | Find references                                             |
|                | `C-c C-j`                     | Jump to a function/class in this buffer (imenu)             |
|                | `C-M-a` / `C-M-e`             | Beginning / end of function                                 |
|----------------+-------------------------------+-------------------------------------------------------------|
| **LSP**        | `M-x lsp-describe-thing-at-point` | Full hover docs                                          |
|                | `M-x lsp-rename`              | Rename a symbol across the project                          |
|                | `M-x lsp-execute-code-action` | Show code fixes (including ruff fixes)                      |
|                | `M-x lsp-workspace-restart`   | Restart the language server                                 |
|                | *(auto)*                      | lsp-mode starts automatically in Python buffers              |
|----------------+-------------------------------+-------------------------------------------------------------|
| **Editing**    | `C-c <` / `C-c >`             | Shift the region left / right one indent level               |
|                | `M-x lsp-format-buffer`       | Format now (ruff)                                            |
|                | *(auto)*                      | Imports sorted and buffer formatted on save                  |
|----------------+-------------------------------+-------------------------------------------------------------|
| **Projects**   | `M-x ygg/python-new-project`  | New uv project: pyproject, =.venv=, dev tools, git repo      |
|                | `M-x pyvenv-activate`         | Activate a venv by hand                                      |
|                | `M-x compile`                 | Run e.g. =uv run main.py= (buffer closes on success)         |
|----------------+-------------------------------+-------------------------------------------------------------|
| **uv** (shell) | `uv add <pkg>`                | Add a dependency (=--dev= for dev-only, e.g. pytest)          |
|                | `uv run <file>`               | Run a script in the project's environment                    |
|                | `uv remove <pkg>`             | Remove a dependency                                           |
|                | `uv tool install <pkg>`       | Install a command-line tool globally (pylsp, ruff)            |
|----------------+-------------------------------+-------------------------------------------------------------|

*Note on =C-c C-p=:* the global =C-c C-p= opens the personal project keymap (see [[*Key Bindings][Key Bindings]]), but in a Python buffer the mode's own =C-c C-p= (=run-python=) takes precedence, so the project menu isn't reachable from there. =C-c C-h= (the project Hydra) is unaffected.
...
;; Sort imports and format through the server (ruff) on save. Errors are
;; demoted to messages: an error in before-save-hook would otherwise abort
;; the save.
(defun ygg/python-lsp-tidy ()
  "Sort imports and format the current buffer via the language server."
  (interactive)
  (when (bound-and-true-p lsp-mode)
    (with-demoted-errors "Python organize imports: %S"
      (lsp-organize-imports))
    (with-demoted-errors "Python format: %S"
      (lsp-format-buffer))))

(defun ygg/python-lsp-tidy-on-save ()
  "Run `ygg/python-lsp-tidy' before saving Python buffers lsp-mode manages."
  (when (derived-mode-p 'python-mode)
    (add-hook 'before-save-hook #'ygg/python-lsp-tidy nil t)))

(add-hook 'lsp-mode-hook #'ygg/python-lsp-tidy-on-save)

(defun ygg/python-new-project (dir)
  "Scaffold a new uv-managed Python project at DIR.

Creates DIR if needed, runs `uv init --app --no-package' there, adds
python-lsp-server and python-lsp-ruff as dev dependencies (which
creates .venv), activates the venv with pyvenv, and opens dired on
DIR.  Runs asynchronously."
  ...
  (let* ((default-directory dir)
         ...
         (script "uv init --app --no-package && uv add --dev python-lsp-server python-lsp-ruff")
         (buf (get-buffer-create "*python-new-project*")))
    (with-current-buffer buf (erase-buffer))
    (make-process
     :name "python-new-project"
     :buffer buf
     :command (list shell-file-name shell-command-switch script)
     :sentinel
     (lambda (proc _event)
       (if (zerop (process-exit-status proc))
           (progn
             ...
             (dired dir)
             (message "ygg/python-new-project: %s ready, venv activated" dir))
         (pop-to-buffer buf)
         (message "ygg/python-new-project: setup failed, see *python-new-project* buffer"))))))
#+end_src
...
| C-c i d         | 13/4/2024                    |
| C-u C-c i d     | 2024-04-13                   |
| C-u C-u C-c i d | Tuesday, April 13, 2024      |
| C-c i t         | ISO 8601 formatted date/time |

#+begin_src emacs-lisp
(defun ygg/insert-date (prefix)
  "Insert the current date (no time) at point.
With no PREFIX argument, use `dd/mm/yyyy' format.  With one
prefix argument (`C-u'), use ISO 8601 date format (`yyyy-mm-dd').
With two prefix arguments (`C-u C-u'), write out the day and
month names in full, e.g. \"Tuesday, April 13, 2024\".

See also `ygg/insert-iso-date-time', which inserts a full ISO
8601 date and time in UTC."
  (interactive "P")
  (let ((format (cond
                 ((not prefix) "%d/%m/%Y")
                 ((equal prefix '(4)) "%F")
...
(defun ygg/insert-iso-date-time ()
  "Insert the current date and time at point, in ISO 8601 format, UTC.
For a date-only alternative, see `ygg/insert-date'."
  (interactive)
  (insert (format-time-string "%FT%T%z" nil "UTC")))

(global-set-key (kbd "C-c i d") 'ygg/insert-date)
(global-set-key (kbd "C-c i t") 'ygg/insert-iso-date-time)
#+end_src
...
**** ygg/title-case-region-or-line
#+begin_src emacs-lisp
(defun ygg/title-case-region-or-line (&optional Begin End)
...
*** Kill Ring
Use Counsel's interactive kill-ring browser instead of the default =M-y= cycling.
#+begin_src emacs-lisp
