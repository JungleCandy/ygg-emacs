...
*** Python
[[https://github.com/astral-sh/uv][uv]] is assumed to be installed on every machine. Install the language server and ruff as global tools, so scripts outside any project still get completion, diagnostics and format-on-save:
...
;; A default font one step smaller than the system default (24px)
(set-face-attribute 'default nil :font "DejaVu Sans Mono:pixelsize=22")
(add-to-list 'default-frame-alist '(font . "DejaVu Sans Mono:pixelsize=22"))
...
;; Make M-x shell start an interactive login bash. (Eshell is pure Elisp and
;; doesn't read shell-file-name/explicit-*-args at all, so this is M-x
;; shell-only, despite appearances.)
(setq shell-file-name "/bin/bash")
(setq explicit-bash-args '("--interactive" "--login"))
...
#+end_src
*** C/C++ Development
`clang`/`clangd` are installed via =apt= on this machine (Crostini/Debian) — install both together with =sudo apt install clang clangd= if missing, or match a versioned =clangd-N= to your =clang= version via =update-alternatives= if the plain package isn't available.

Uses `eglot` (built into Emacs), the same as [[*Rust][Rust]] and [[*Python Development][Python]] — it only starts a server when =clangd= is actually found on =$PATH=, so this is safe to keep even on a machine without it installed:
#+begin_src emacs-lisp
(defun my/c-eglot-if-available ()
  (when (executable-find "clangd")
    (eglot-ensure)))

(use-package eglot
  :ensure nil
  :hook ((c-mode . my/c-eglot-if-available)
         (c++-mode . my/c-eglot-if-available)))
#+end_src

Eldoc's echo-area hints truncate to one line by default. For the full multiline doc, run =M-x eldoc-doc-buffer= (bound to =C-c f H=) to pop it into a dedicated =*eldoc*= buffer.

Indentation style is One True Brace Style (OTBS/1TBS) — every opening brace, including function definitions, stays cuddled at the end of the line, and `else`/`while`/`catch` cuddle onto the preceding closing brace. `cc-mode` has no built-in style for this (the closest, =linux=, still puts a function's opening brace on its own line), so it's defined as a custom style based on =linux=. This doesn't depend on =clangd= at all, so it applies regardless:
#+begin_src emacs-lisp
(use-package cc-mode
  :ensure nil
  :config
...
(when (and (fboundp 'treesit-available-p) (treesit-available-p))
  (unless (treesit-language-available-p 'rust)
    (add-to-list 'treesit-language-source-alist
                 ;; Pinned to v0.23.3: the last tag generating ABI 14 grammar
                 ;; code. v0.24.0+ generates ABI 15, which Emacs 30.1's bundled
                 ;; tree-sitter runtime rejects (version-mismatch).
...
Everything runs through one language server: =pylsp= with the
...
There are two ways to work, and Eglot handles both the same way:
...
On save, =ygg/python-eglot-tidy= sorts imports (=eglot-code-action-organize-imports=)
and then formats the buffer (=eglot-format-buffer=), both through ruff. Formatting
uses ruff's defaults (88 columns, black-compatible style); change them per project
under =[tool.ruff]= in =pyproject.toml=. Errors are demoted to messages, because an
error in =before-save-hook= would otherwise abort the save; an already-sorted file
(where Eglot reports no organize-imports action) is silent.

Like C and Rust, Eglot only starts when a server (=pylsp=) is actually found on
=exec-path=. The hook is appended so it runs after =ygg/auto-pyvenv-activate= has
put the project's =.venv/bin= on the path.

*Keybindings and commands.* Everything here is standard =python.el=, Xref, or Eglot — nothing is custom-bound except where noted.
...
|----------------+-------------------------------+-----------------------------------------------------------|
| **Navigation** | `M-.`                         | Jump to definition (Eglot)                                |
|                | `M-,`                         | Jump back                                                 |
|                | `M-?`                         | Find references                                           |
|                | `C-c C-j`                     | Jump to a function/class in this buffer (imenu)           |
|                | `C-M-a` / `C-M-e`             | Beginning / end of function                               |
|----------------+-------------------------------+-----------------------------------------------------------|
| **Eglot**      | `M-x eldoc-doc-buffer`        | Full hover docs in a dedicated buffer                     |
|                | `M-x eglot-rename`            | Rename a symbol across the project                        |
|                | `M-x eglot-code-actions`      | Show code fixes (including ruff fixes)                    |
|                | `M-x eglot-reconnect`         | Restart the language server                               |
|                | *(auto)*                      | Eglot starts once =pylsp= is found in the venv            |
|----------------+-------------------------------+-----------------------------------------------------------|
| **Editing**    | `C-c <` / `C-c >`             | Shift the region left / right one indent level            |
|                | `M-x eglot-format-buffer`     | Format now (ruff)                                         |
...
;; Start Eglot only if pylsp is present. Appended so it runs after the
;; venv is activated and pylsp can be found in .venv/bin.
(defun my/python-eglot-if-available ()
  (when (executable-find "pylsp")
    (eglot-ensure)))

(add-hook 'python-mode-hook #'my/python-eglot-if-available t)

;; Sort imports and format through the server (ruff) on save. Errors are
;; demoted to messages, one step at a time: an error in before-save-hook would
;; abort the save. "No organize-imports action" (already sorted) stays silent.
(defun ygg/python-eglot-tidy ()
  "Sort imports and format the current buffer via the language server."
  (when (eglot-managed-p)
    (condition-case err
        (eglot-code-action-organize-imports (point-min))
      (error
       ;; Already sorted: Eglot reports "No ... code actions here".
       (unless (string-match-p "code actions here" (error-message-string err))
         (message "Python organize imports: %S" err))))
    (with-demoted-errors "Python format: %S"
      (eglot-format-buffer))))

(defun ygg/python-eglot-tidy-on-save ()
  "Run `ygg/python-eglot-tidy' before saving Python buffers Eglot manages."
  (when (and (eglot-managed-p) (derived-mode-p 'python-mode 'python-ts-mode))
    (add-hook 'before-save-hook #'ygg/python-eglot-tidy nil t)))

(add-hook 'eglot-managed-mode-hook #'ygg/python-eglot-tidy-on-save)
