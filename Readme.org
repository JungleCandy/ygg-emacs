...
To set up a new project, prefer [[https://github.com/astral-sh/uv][uv]] if it's installed on this machine —
it hardlinks packages from a shared cache instead of duplicating them per
venv, which keeps multi-project disk usage sane. Fall back to the stdlib
=venv= module if =uv= isn't available; either way the result is a normal
=.venv= directory, so Emacs behaves identically regardless of which one
...
#+begin_src shell
  ...
  if command -v uv >/dev/null 2>&1; then
    ...
    uv pip install "python-lsp-server[all]" black isort
  else
    python3 -m venv .venv
    source .venv/bin/activate
