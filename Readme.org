...
*** Python Development
Each project gets its own venv at its root; =ygg/auto-pyvenv-activate= (below)
finds and activates it automatically when a =.py= file is opened, so there's
no need to run =pyvenv-workon= or =pyvenv-activate= by hand.

To set up a new project, run =M-x ygg/python-new-project= and give it a
directory. It creates the directory if needed, then builds the venv with
[[https://github.com/astral-sh/uv][uv]] if it's installed on this machine — uv hardlinks packages from a shared
cache instead of duplicating them per venv, which keeps multi-project disk
usage sane — falling back to the stdlib =venv= module otherwise. Either way
the result is a normal =.venv= directory, so Emacs behaves identically
regardless of which one made it. It installs =python-lsp-server=,
=python-lsp-ruff=, =black=, and =isort= into the new venv, activates it via =pyvenv=, and opens
=dired= on the project root, all asynchronously so Emacs doesn't block.

The server is deliberately bare =python-lsp-server= plus the
[[https://github.com/python-lsp/python-lsp-ruff][python-lsp-ruff]] plugin, not =python-lsp-server[all]=. The =[all]= extra pulls in
pylint, flake8, rope, yapf, autopep8 and friends, which duplicate what ruff
already does (linting) or what =black=/=isort= already do (formatting). Jedi
still provides completion, hover, definitions, references and rename.

That's it — =eglot=, =blacken-mode=, and =py-isort= all pick up the
venv's binaries automatically via the hook, whether =uv= made it or not.
Eglot doesn't format on save by itself, so the LSP server never fights
=blacken-mode= over formatting. =py-isort= runs with =--profile=black= so
isort and black agree on how to wrap imports.

Like C and Rust, Eglot only starts when a server (=pylsp=) is actually found
on =exec-path=. The hook is appended so it runs after
=ygg/auto-pyvenv-activate= has put the project's =.venv/bin= on the path.
...
|                | *(auto)*                      | black + isort run on save                                 |
|----------------+-------------------------------+-----------------------------------------------------------|
| **Projects**   | `M-x ygg/python-new-project`  | Create a directory with a =.venv= and the tools           |
|                | `M-x pyvenv-activate`         | Activate a venv by hand                                   |
|                | `M-x compile`                 | Run e.g. =uv run main.py= (buffer closes on success)      |
|----------------+-------------------------------+-----------------------------------------------------------|
| **uv** (shell) | `uv init`                     | Add a =pyproject.toml= to the current directory           |
|                | `uv add <pkg>`                | Add a dependency (=--dev= for dev-only, e.g. pytest)      |
|                | `uv run <file>`               | Run a script in the project's environment                 |
|                | `uv pip install <pkg>`        | Install into the active =.venv= without touching pyproject|
|----------------+-------------------------------+-----------------------------------------------------------|
...
(defun ygg/python-new-project (dir)
  "Scaffold a new Python project at DIR.

Creates DIR if needed, builds a .venv there via uv (if installed)
or the stdlib venv module otherwise, installs
python-lsp-server/python-lsp-ruff/black/isort into it, activates the venv
with pyvenv, and opens dired on DIR. Runs asynchronously."
  ...
  (let* ((default-directory dir)
         ...
         (script
          (if (executable-find "uv")
              (format "uv venv %s && uv pip install --python %s/bin/python python-lsp-server python-lsp-ruff black isort"
                      (shell-quote-argument venv) (shell-quote-argument venv))
            (format "%s -m venv %s && %s/bin/pip install python-lsp-server python-lsp-ruff black isort"
                    (shell-quote-argument (or (executable-find "python3") "python3"))
